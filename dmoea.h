...
public:
	...
	void reactToEnviChange(vector<double> &igdValue, vector<double> &hvValue);
	...
	void construct_train_set(vector<vector<double> > sample_x, vector<vector<double> > sample_y);
...
int TMOEAD::dominate_comp(const vector<double>& a, const vector<double>& b) {
...
double TMOEAD::cal_distance(const vector<double>& a, const vector<double>& b) {
	double d = 0;
	for (int i = 0; i < a.size(); ++i) {
		d += (a[i] - b[i]) * (a[i] - b[i]);
	}
...
void TMOEAD::cal_maxmin_value(vector<double>& fmin, vector<double>& fmax) {
...
void TMOEAD::update_EP(const TIndividual& child) {
...
void TMOEAD::init_uniformweight(int sd)
{
	...
	//double b = 1.0 / p;
	int top = -1;
	vector<int> stack(DynPara::objNum);
	vector<int> ysum(DynPara::objNum);
	vector<int> visit(DynPara::objNum);
	...
	while (top != -1) {
		...
		if (top == DynPara::objNum - 1) {
			...
			if (!visit[top + 1] && v == ysum[top]) {
				...
				top++;
				stack[top] = 0;
				for (int j = top + 1; j < DynPara::objNum; ++j) {
					visit[j] = false;
				}
			}
...
bool TMOEAD::enviChangeNextFes(int curFes) {
	//
	if (DynPara::change_nt_ct) {
		if (curFes <= 0) return false;
		int ass_num = 0;
...
int TMOEAD::getEnviIndex(int curFes) {
	if (DynPara::change_nt_ct) {
		if (curFes <= 0) return false;
		int ass_num = 0;
		int eindex = -1;
...
void TMOEAD::introduceDynamic(int curFes) {
	if (DynPara::test_SDP) {
	...
	if (DynPara::test_DF && DynPara::change_nt_ct) {
		int ass_num = 0;
		int eindex = -1;
		for (int j = 0; j < DynPara::enviNum; ++j) {
			eindex = j;
		...
		eindex = eindex + 1;
		if (eindex >= DynPara::enviNum) return;
		DynPara::nt = DynPara::change_nt[eindex];
		DynPara::changeFre = 100*DynPara::change_ct[eindex];
	}
}
bool TMOEAD::detectChange(int i, vector<double>& igdValue, vector<double>& hvValue) {
...
void TMOEAD::reactToDimChange(bool varDimChange, bool objDimChange) {
	//parameter change 
	if (objDimChange) {
		/*int pointNum = 23;
...
void TMOEAD::storePSForDetectChange_DE() {
...
void TMOEAD::print_set(const vector<CSolution>& set) {
...
void TMOEAD::construct_train_set(vector<vector<double> > sample_x, vector<vector<double> > sample_y) {

}

void TMOEAD::cal_change_past_envi(vector<solpair>& set, vector<vector<int> >& select_indepvar, int start_envi, vector<int>& self_variables) {
	...
	if (InfoMemory::detectedEnviSol.size() >= 2) {
		...
		vector<vector<int> > pairs = pair_sol_detected_change(InfoMemory::detectedEnviSol[j], InfoMemory::detectedEnviSol[j - 1], weights, use_ep);
		//cout << pairs.size() << "\t";
...
void TMOEAD::init_pop_correlation() {
	int cur_envi = InfoMemory::detectedEnviSol.size();
	int num_dim = DynPara::dimNum;
...
void TMOEAD::obtain_previous_data(vector<CSolution>& data) {
	int num_dim = DynPara::dimNum;
	int num_obj = DynPara::objNum;
...
void TMOEAD::obtain_new_data(vector<CSolution>& data, vector<double>& IGD, vector<double>& HV) {
	int num_dim = DynPara::dimNum;
	int num_obj = DynPara::objNum;
...
int TMOEAD::select_pre_cluster(Cluster& new_clst, Cluster& old_clst, int j, const vector<int>& visit) {
	int num_obj = DynPara::objNum;
...
vector<CSolution> TMOEAD::define_knee_in_set(const vector<CSolution>& set, vector<int>& subspace_index, vector<int>& label) {
	int subspace = 10;
...
vector<vector<double> > TMOEAD::estimate_new_knee_point(const vector<CSolution>& a, const vector<int>& aindex, const vector<CSolution>& b, const vector<int>& bindex) {
...
vector<CSolution> TMOEAD::generate_source_knee_data(const vector<CSolution>& set, const vector<int>& label) {
	int num_dim = DynPara::dimNum;
	int num_obj = DynPara::objNum;
	vector<vector<double> > noise_sol(pops);
...
vector<CSolution> TMOEAD::generate_target_knee_data(const vector<vector<double> >& estimated_knee) {
	int num_dim = DynPara::dimNum;
	int num_obj = DynPara::objNum;
	vector<vector<double> > noise_sol(pops);
	vector<int> noise_label(pops);
...
void TMOEAD::init_pop_use_sol(const vector<vector<double> >& predict_knee, vector<double>& IGD, vector<double>& HV) {
	for (int j = 0; j < pops; ++j) {
		if (enviChangeNextFes(mEvas) && mEvas < DynPara::totalFes) {
			metric(IGD, HV);
			storeFinalPop();
...
void TMOEAD::transfer_knee_SVM(vector<double>& IGD, vector<double>& HV) {
	
}
void TMOEAD::transfer_cluster_SVM(vector<double>& IGD, vector<double>& HV) {
	
}

void TMOEAD::presearch(vector<double>& IGD, vector<double>& HV, vector<int>& label, vector<vector<double> >& set) {
...
void TMOEAD::transfer_ind_SVM(vector<double>& IGD, vector<double>& HV) {
	
}
void TMOEAD::predict_by_SVR(vector<double>& IGD, vector<double>& HV) {
	
}

vector<int> TMOEAD::rank_sol_crowd_distance(const vector<CSolution>& set) {
...
void TMOEAD::init_half_pop_by_predict(const vector<CSolution>& predict_sol, vector<double>& IGD, vector<double>& HV) {
...
void TMOEAD::predict_by_AE(vector<double>& IGD, vector<double>& HV) {
	
}

//使用神经网络预测解
void TMOEAD::predict_by_neuralnetwork(vector<double>& IGD, vector<double>& HV) {
...
void TMOEAD::reevaluate_detector() {
...
void TMOEAD::gaussian_pop(vector<double>& igdValue, vector<double>& hvValue, double rate) {
...
//初始化种群规模
void TMOEAD::reinitialize_pop(vector<double>& igdValue, vector<double>& hvValue, double rate) {
	//randomly select 10% inviduals to be reinitialized
	int num = pops * rate;
	vector<int> slist(pops);
	for (int j = 0; j < pops; ++j) {
...
void TMOEAD::reevaluate_pop(vector<double>& igdValue, vector<double>& hvValue) {
	for (int j = 0; j < population.size(); ++j) {
		if (enviChangeNextFes(mEvas) && mEvas < DynPara::totalFes) {
			metric(igdValue, hvValue);
			storeFinalPop();
...
vector<double> TMOEAD::estimate_error(vector<double>& IGD, vector<double>& HV) {
	int num_obj = DynPara::objNum;
	int num_dim = DynPara::dimNum;
...
void TMOEAD::reinit_hist_center(vector<double>& IGD, vector<double>& HV) {
	//回退历史解
	int num_obj = DynPara::objNum;
	int num_gen = 10;
...
void TMOEAD::reactToEnviChange(vector<double>& igdValue, vector<double>& hvValue) {
...
double TMOEAD::cal_pop_error_to_optima() {
	double error = 0;
	int num_dim = DynPara::dimNum;
	for (int i = 0; i < pops; ++i) {
...
vector<vector<double> > TMOEAD::correct_sol_by_predict_error(const vector<vector<double> >& set, const vector<CSolution>& osol, const vector<double>& train_error, const vector<int>& self_variable, const vector<int>& use_correct) {
...
vector<double> TMOEAD::eval_prediction_error() {
	int num_dim = DynPara::dimNum;
...
vector<double> TMOEAD::eval_center_move() {
	int num_dim = DynPara::dimNum;
...
vector<double> TMOEAD::eval_move_error() {
	int num_dim = DynPara::dimNum;
	vector<double> error(num_dim, 0);
...
void TMOEAD::evolution(vector<double>& igdValue, vector<double>& hvValue)
{
	//bool use_ep = false;
	//每一代10个随机个体进行环境变化检测
	...
	for (int i = 0; i < population.size(); i++)
	{
		if (useDetection && i % 20 == 0) {
			...
			if (enviHasChanged) {
				...
				reactToEnviChange(igdValue, hvValue);
			}
		}
		int enviIndex = getEnviIndex(mEvas);
		if (enviChangeNextFes(mEvas) && (mEvas < DynPara::totalFes )) {  //|| enviIndex + 1 < DynPara::enviNum
			metric(igdValue, hvValue);
			storeFinalPop();
			int preDimNum = DynPara::dimNum; int preObjNum = DynPara::objNum;
			introduceDynamic(mEvas);
...
void TMOEAD::run(int sd, int nc, int maxfes, int rn, vector<double>& igdValue, vector<double>& hvValue)
{
	...
	while (mEvas < maxfes) {
		...
		int enviIndex = getEnviIndex(mEvas);
		//cout << mEvas << "\t" << enviIndex << "\t" << DynPara::totalFes << "\n";
	}
	metric(igdValue, hvValue);
	int enviIndex = getEnviIndex(mEvas);
...
void TMOEAD::storeFinalPop() {
	...
	int curenvi = getEnviIndex(mEvas);
	if (InfoMemory::algSolAllEnvi.size() != curenvi + 1) assert(false);
	//record_metric();
}
void TMOEAD::storeInitPop() {
	vector<CSolution> set;
	for (int j = 0; j < pops; ++j) {
		CSolution s;
		s.x = population[j].indiv.x_var;
...
void TMOEAD::storeAllPop(int runId) {
	stringstream objstr;
...
void TMOEAD::record_metric() {
	if (mEvas == 1 || mEvas % InfoMemory::sampleFre == 0 || (mEvas - DynPara::firstEnviFes)% DynPara::changeFre == 1) {
...
void TMOEAD::metric(vector<double>& igdValue, vector<double>& hvValue) {
	int enviIndex = getEnviIndex(mEvas);
	//vector<vector<double> > oldObjValue(pops);
	//int nobj = DynPara::objNum;
	/*
...
double TMOEAD::calculateIGD(const vector<CSolution>& set, const int enviIndex, vector<double>& igdValue, bool update) {
		...
		for (int i = 0; i < set.size(); ++i) {
			...
			if (smaller > 0 && larger == 0) {
				...
				if (false) {
					...
					//string tstr = "//taut" + tautstr.str() + "_nt" + ntstr.str() + "//";
					if (DynPara::change_nt_ct) tstr = "//changing_taut_nt//";
	...
	//cout << value << "\t";
	if (update) {
		if (igdValue.size() < enviIndex + 1) assert(false);
		igdValue[enviIndex] = value;
	}
	if (pfisworse) cout << "+";
	return value;
}
void TMOEAD::readReferPointForIGD(int enviIndex) {
	//read the reference points from the file
	stringstream objStr;
	objStr << DynPara::initObjNum;
	stringstream enviStr;
	enviStr << 31; // DynPara::enviNum;
	//string proName = strTestInstance;
	if (DynPara::test_SDP) {
...
void TMOEAD::readReferPointForHV(int enviIndex, vector<double>& v) {
	stringstream objStr;
	objStr << DynPara::initObjNum;
	stringstream enviStr;
...
double TMOEAD::calculateHV(const vector<CSolution>& set, const int enviIndex, vector<double>& hvValue, bool update) {
	if (population.size() == 0) assert(false);
...
vector<double> TMOEAD::cal_pop_center() {
	int num_dim = DynPara::dimNum;
	vector<double> center(num_dim, 0);
	for (int j = 0; j < pops; ++j) {
		for (int k = 0; k < num_dim; ++k) {
...
void TMOEAD::resample_pop(const vector<vector<double> >& sample, vector<double>& igdValue, vector<double>& hvValue) {
	for (int j = 0; j < pops; ++j) {
		if (enviChangeNextFes(mEvas) && mEvas < DynPara::totalFes) {
			metric(igdValue, hvValue);
			storeFinalPop();
...
void TMOEAD::outputVector(const vector<double> x) {
	cout << "(";
