...
namespace esvm {
	...
	void SVMClassifier::train(const Eigen::MatrixXf &X, const Eigen::MatrixXf &y, const vector<double> &weight) {
	...
	void SVMClassifier::train(const Eigen::MatrixXf &X, const vector<int> &y, const vector<double> &weight) {
		...
		for (int i = 0; i < Ntr; i++) { problem_->W[i] = weight[i]; }
	...
	void SVMRegression::train(const Eigen::MatrixXf &X, const Eigen::MatrixXf &y, const vector<double> &weight) {
	...
	void SVMRegression::train(const Eigen::MatrixXf &X, const vector<double> &y, const vector<double> &weight) {
	...
	void SVMRegression::test(const Eigen::MatrixXf &X, vector<double> &yhat) {
	...
	void SVMRegression::getw(Eigen::MatrixXf &w, float &b) {
		if (model_ == NULL) { printf("Error! Train an SVM first! Exitting... \n"); return; }
	...
	int SVMRegression::saveModel(const char *filename) {
		int result = svm_save_model(filename, model_);
		return result;
	}

	void SVMRegression::loadModel(const char *filename) {
		if (model_ != NULL) svm_free_model_content(model_); // deallocate first
		model_ = svm_load_model(filename);
	}

	void SVMRegression::setC(double Cnew) {
		param_->C = Cnew;
	}

	void SVMRegression::setPara(double C, double epsion, double gamma) {
		param_->C = C;
		param_->eps = epsion;
		param_->gamma = gamma;
	}
