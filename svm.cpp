...
public:
	...
	static double k_function(const svm_node *x, const svm_node *y,
		const svm_parameter& param, int D=0, int dy=0);
	virtual Qfloat *get_Q(int column, int len) const = 0;
	virtual double *get_QD() const = 0;
...
double Kernel::k_function(const svm_node *x, const svm_node *y,
	const svm_parameter& param, int D, int dy)
{
	...
	case RBF:
	{
		...
		int i = 0;
		int j = 0;
		while (i < D && j < dy && x->index != -1 && y->index != -1)
		{
			if (x->index == y->index)
			{
...
double svm_predict_values(const svm_model *model, const svm_node *x, double* dec_values)
{
		...
		model->param.svm_type == NU_SVR)
	{
		...
		double sum = 0;
		int D = sizeof(x)/sizeof(svm_node*);
		int ny = sizeof(model->SV[i]) / sizeof(svm_node*);
		for (i = 0; i < model->l; i++)
			sum += sv_coef[i] * Kernel::k_function(x, model->SV[i], model->param, D, ny);
		sum -= model->rho[0];
		*dec_values = sum;
